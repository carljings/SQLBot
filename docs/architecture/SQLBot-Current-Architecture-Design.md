...
│  │  │  - Swagger 文档                           │   │
│  │  │  │  │  │  - 注：前端通过 npm run build 打包为静态文件，由 FastAPI 静态文件服务提供 │   │   │
│  │  └─────────────────────────────────────────────┘   │   │
│  │  ┌─────────────────────────────────────────────┐   │   │
│  │  │  MCP 服务器 (port 8001)              │   │   │
...
**开发模式**:
- 前端: Vite Dev Server (5173) ← 后端 API (8000)
- 热重载、TypeScript 编译、ESLint

**部署说明**:
- **前端开发**: Vite Dev Server (5173) → 后端 API (8000)，支持热重载
- **生产部署**: Docker 容器内运行所有服务
  - 前端打包后由 FastAPI 静态服务 (8000)
  - g2-ssr (3000) 和 PostgreSQL (5432) 运行在同一容器内
