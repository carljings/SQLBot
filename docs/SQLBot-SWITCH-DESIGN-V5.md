# SQLBot 双方案切换设计（V5 - 三端职责明确版）

## 📋 概要设计

### 1. 设计原则

按照SQLBot现有架构模式设计，明确三端职责划分：
...
【前端】用户问题
    ↓
【SQLBot后端】RAG检索（embedding向量）
    ├── 检索表结构
    ├── 检索术语库
    └── 检索SQL示例
    ↓
【SQLBot后端】调用LLM API生成SQL
    ↓
【SQLBot后端】执行SQL
    ↓
【SQLBot后端】生成图表配置
    ↓
【SQLBot后端】返回结果（SQL + 数据 + 图表配置）
    ↓
【前端】展示结果
```
...
| 端 | LLM方案 | Claude Code方案 |
|----|---------|-----------------|
| **Claude Code** | 不涉及 | 读取MD + 生成SQL |
| **SQLBot后端** | RAG + LLM生成 + 执行 + 图表 + 返回 | 执行 + 图表 + 返回 |
| **前端** | 展示结果 | 展示结果 |

**详细职责对比**：

| 功能 | LLM方案 | Claude Code方案 |
|------|---------|-----------------|
| **RAG检索** | SQLBot后端（embedding） | 跳过 |
| **读取上下文** | SQLBot后端检索 | Claude Code读MD |
| **SQL生成** | SQLBot后端（LLM API） | Claude Code |
| **SQL执行** | SQLBot后端 | SQLBot后端 |
| **图表配置** | SQLBot后端 | SQLBot后端 |
| **返回数据** | SQLBot后端 | SQLBot后端 |
| **展示结果** | 前端 | 前端 |
...
```
前端 → SQLBot后端 → LLM API → SQLBot后端 → 前端
       ↑             ↓          ↑         ↓
       └─ RAG检索 ───┘          └─ 执行 + 图表 ─┘
```
...
```
前端 → SQLBot后端 → Claude Code → SQLBot后端 → 前端
                        ↑            ↓
                        └─ 读MD生成SQL ─┘
                                          ↓
                                     执行 + 图表
...
class LLMSQLGenerator(BaseSQLGenerator):
    ...
    职责：
    - SQLBot后端RAG检索（embedding向量）
    - SQLBot后端调用LLM API生成SQL
    - SQLBot后端执行SQL
    - SQLBot后端生成图表配置
    - SQLBot后端返回结果
    - 前端展示结果
...
**效果**：
- SQLBot后端完成所有逻辑（RAG + SQL生成 + 执行 + 图表 + 返回）
- 前端展示结果
...
| 优势 | 说明 |
|------|------|
| ✅ **三端职责明确** | Claude Code生成SQL，SQLBot执行+图表，前端展示 |
